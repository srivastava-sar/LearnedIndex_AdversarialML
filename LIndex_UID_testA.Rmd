...
```{r readData}
DIGIX <- read.csv(file = "datasets/test_data_A-uid.csv")
str(DIGIX)
attach(DIGIX)
```
...
<<Polyfit_Visual>>
Quadfunct = func_PolyVis(X,2,uid,DIGIX)
```
...
maxQ <- which(Quadfunct$residuals^2>80000)
#Max k residuals
#print(maxQ)
print(maxQ)
print(length(maxQ))
...
outcomeExits = func_CUSUM2(FIT,5,keyDiff,keyDim,y_lim=100)
print(outcomeExits)
<<PlotRes>>
...
outcomeExits = func_CUSUM_wrapper(FIT,5,keyDiff,keyDim,y_lim=100)
...
for (l in 1:10){
  for (t in 1:9){
    outcomeExits = func_CUSUM_wrapper(FIT,T_factor[t],keyDiff,keyDim,y_lim=100,FALSE)
